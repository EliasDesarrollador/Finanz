...
const Auth = () => {
  ...
  useEffect(() => {
    // Procesar retorno de OAuth: buscamos #sso=base64url({ user })
    try {
      ...
      if (m && m[1]) {
        ...
        if (json && json.user) {
          localStorage.setItem('et_user', JSON.stringify(json.user));
          navigate('/dashboard');
        }
      }
    } catch {}
  ...
  const startFacebook = () => {
    const redirect = window.location.origin + '/auth';
    window.location.href = `${API_BASE}/api/auth/facebook/start?redirect=${encodeURIComponent(redirect)}`;
  };
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="p-8 bg-gradient-card shadow-elegant animate-fade-in">
          ...
          <div className="space-y-3">
            <Button variant="outline" className="w-full" onClick={startGoogle}>
            ...
            <Button variant="outline" className="w-full" onClick={startFacebook}>
